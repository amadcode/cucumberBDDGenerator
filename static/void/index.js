    ...
    let array = ""
        ...
        for (let i = 0 ; i < array.length; i++) {
            ...
            array[i] = atTheRate
                        ...
                        + methodGeneratorStepDefinition_SecondLayer(step[1])  // ref:01 this function is to generate inner step generation body/ core component method
        }
    stepDefinition.value = array.join("\n")

        /// cc work

    let ab = localStorage.getItem("featureSteps")
    let arrayb = ""
        arrayb = ab.split('\n')
        for (let i = 0 ; i < array.length; i++) {
...
// ref:01
// this function is to generate inner step generation body/ core component method
function methodGeneratorStepDefinition_SecondLayer(array) {
    underscoredArray = array.replaceAll(" ", "_").replace(/"([^"]+)"/g, dynamicValue).replace(/'([^']+)'/g, dynamicValue);
...
function methodGeneratorInnerStepDefinitionBody_ThirdLayer(array, noOfDynamicValues) {
    ...
    methodInnerBlock = methodInnerBlock.replace(/"([^"]+)"/g, dynamicValue_).replace(/'([^']+)'/g, dynamicValue_);
    methodInnerBlock = "\n"
                            + "\t"
                            + "// "
                            + methodInnerBlock
